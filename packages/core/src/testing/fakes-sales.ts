...
export class InMemoryOrderRepository implements OrderRepository {
  ...
  async searchOrders(
    ...
    const matched = this.orders.filter((o) => {
      ...
      if (q) {
        const haystack = [o.code, o.buyerEmail, o.buyerName, o.buyerDocument ?? ""]
          .join(" ")
          .toLowerCase();
    ...
    // Most recent first — insertion order is the proxy for createdAt here.
    return matched
      .map((o, i) => ({ o, i: this.orders.indexOf(o) }))
      .sort((a, b) => b.i - a.i)
  ...
  async aggregatePaidByBuyer(organizationId: string, eventId?: string) {
    ...
    for (const o of this.orders) {
      if (o.organizationId !== organizationId || o.status !== "PAID") continue;
      if (eventId && o.eventId !== eventId) continue;
      const row = map.get(o.buyerEmail) ?? {
        buyerEmail: o.buyerEmail,
        orderCount: 0,
        totalCents: 0,
      };
...
export class FakePsp implements PspPort {
  ...
  /** Authoritative status returned by getTransaction (reconciliation tests). */
  nextTransactionStatus: PspTransactionStatus | null = null;
  readonly transactionCalls: string[] = [];
  /** Status returned by the next createCardCharge (card flow tests). */
  nextCardStatus: PspTransactionStatus = "approved";
  readonly cardCalls: CreateCardChargeInput[] = [];
  private counter = 0;
  ...
  async createCardCharge(input: CreateCardChargeInput): Promise<CardCharge> {
    ...
    this.counter += 1;
    return { providerTransactionId: `mpc_${this.counter}`, status: this.nextCardStatus };
  }
  ...
  async getTransaction(providerTransactionId: string): Promise<PspTransaction> {
    this.transactionCalls.push(providerTransactionId);
    return {
      providerTransactionId,
      status: this.nextTransactionStatus ?? "pending",
      amount: 0 as never,
    };
  }
