...
export const POST = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const coupon = await (await getTenantServices(params.orgId)).promoters.createCoupon(ctx, params.eventId, input);
...
export const GET = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const coupons = await (await getTenantServices(params.orgId)).promoters.listCoupons(ctx, params.eventId);
