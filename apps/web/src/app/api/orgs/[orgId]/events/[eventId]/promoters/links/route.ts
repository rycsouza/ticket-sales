...
export const POST = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const link = await (await getTenantServices(params.orgId)).promoters.createLink(ctx, params.eventId, input);
...
export const GET = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const links = await (await getTenantServices(params.orgId)).promoters.listLinks(ctx, params.eventId);
