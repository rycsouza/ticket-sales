...
export const POST = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const batch = await (await getTenantServices(params.orgId)).inventory.createSalesBatch(ctx, params.eventId, input);
...
export const GET = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const batches = await (await getTenantServices(params.orgId)).inventory.listSalesBatches(ctx, params.eventId);
