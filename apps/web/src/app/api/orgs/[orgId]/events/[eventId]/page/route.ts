...
export const GET = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const page = await (await getTenantServices(params.orgId)).eventPage.getPage(ctx, params.eventId);
...
export const PUT = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const page = await (await getTenantServices(params.orgId)).eventPage.updatePage(ctx, params.eventId, input);
