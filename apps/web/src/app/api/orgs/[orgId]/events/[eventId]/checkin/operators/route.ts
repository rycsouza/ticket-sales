...
export const POST = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    ...
    const assignment = await (await getTenantServices(params.orgId)).checkin.assignOperator(ctx, params.eventId, input);
...
export const GET = route<{ orgId: string; eventId: string }>(
  async (request, { params, correlationId }) => {
    const ctx = await requireOrgContext(request, params.orgId, correlationId);
    const operators = await (await getTenantServices(params.orgId)).checkin.listOperators(ctx, params.eventId);
