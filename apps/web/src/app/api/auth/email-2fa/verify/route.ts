...
/** Second factor at login via a one-time code sent to the account e-mail. */
export const POST = route(async (request, { correlationId }) => {
  ...
  const result = await getPlatformServices().auth.verifyEmailOtp(input.challengeToken, input.code, meta, {
